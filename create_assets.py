...
def create_explosion_sound(filename, duration=0.5, volume=0.4):
    ...
    with wave.open(filename, 'w') as wav_file:
        ...
        for i in range(num_samples):
            ...
            boom_freq = 80 + (time * 20)  # Frequency sweep
            boom_sample = math.sin(2 * math.pi * boom_freq * time) * 0.4
            
            # Mid frequency crackle
            crackle_freq = 200 + (time * 100)
            crackle_sample = math.sin(2 * math.pi * crackle_freq * time) * 0.3
            
            # High frequency sizzle
            sizzle_freq = 800 + (time * 400)
            ...
            # Combine all layers
            sample = boom_sample + crackle_sample + sizzle_sample + noise
            
            # Apply explosion envelope (quick attack, slow decay)
            envelope = 1.0
            if time < 0.05:  # Quick attack
                envelope = time / 0.05
            else:  # Slow decay
                envelope = math.exp(-(time - 0.05) * 8)
...
def create_audio_files():
    ...
    create_explosion_sound('assets/explosion.wav', duration=0.5, volume=0.4)
