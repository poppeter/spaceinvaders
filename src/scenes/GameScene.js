...
export default class GameScene extends Phaser.Scene {
  ...
  handlePlayerHit(playerSprite, bullet) {
    ...
    if (this.lives <= 0) {
      ...
      // Set a flag to pause game elements
      this.isPausedForRespawn = true;
      
      // Respawn player after a longer delay
      this.player.respawn();
      
      // Resume game after explosion sound finishes (about 2 seconds)
  ...
  handleBarrierHit(bullet, barrier) {
    ...
    if (barrier && typeof barrier.damage === 'function') {
      barrier.damage();
      this.sound.play('barrier_explosion');
    }
  }
  ...
  update(time, delta) {
    ...
    // Only update game elements if not paused for respawn
    if (!this.isPausedForRespawn) {
